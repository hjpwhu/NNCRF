...
public:
  ...
  inline void initial(int nOSize, int nLISize, int nRISize, bool bUseB=true, int seed = 0, int funcType = 0) {
     ...
     random(_WR, -1.0 * bound, 1.0 *bound, seed);
   ...
   inline void initial(Tensor<xpu, 2, double> WL, Tensor<xpu, 2, double> WR, Tensor<xpu, 2, double> b,
...
public:
  ...
  inline void ComputeBackwardLoss(Tensor<xpu, 2, double> xl, Tensor<xpu, 2, double> xr,
      Tensor<xpu, 2, double> y, Tensor<xpu, 2, double> ly,
