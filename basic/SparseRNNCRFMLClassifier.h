...
public:
  ...
  inline double process(const vector<Example>& examples, int iter) {
    ...
    for (int count = 0; count < example_num; count++) {
      ...
      Tensor<xpu, 2, double> rnn_hidden_left[seq_size][_rnnMidLayers + 1], rnn_hidden_leftLoss[seq_size][_rnnMidLayers + 1],
          rnn_hidden_leftFLoss[seq_size][_rnnMidLayers + 1];
      Tensor<xpu, 2, double> rnn_hidden_right[seq_size][_rnnMidLayers + 1], rnn_hidden_rightLoss[seq_size][_rnnMidLayers + 1],
          rnn_hidden_rightFLoss[seq_size][_rnnMidLayers + 1];
      ...
      for (int idx = seq_size - 1; idx >= 0; idx--) {
        ...
        if (idx == 0)
          _rnn_left.ComputeBackwardLoss(rnn_hidden_null, input[idx], rnn_hidden_left[idx][0], rnn_hidden_leftLoss[idx][0], rnn_hidden_nullLoss, inputLoss[idx]);
        else
          _rnn_left.ComputeBackwardLoss(rnn_hidden_left[idx - 1][0], input[idx], rnn_hidden_left[idx][0], rnn_hidden_leftLoss[idx][0],
              rnn_hidden_leftFLoss[idx - 1][0], inputLoss[idx]);
      }
      ...
      for (int idx = 0; idx < seq_size; idx++) {
        ...
        if (idx == seq_size - 1)
          _rnn_right.ComputeBackwardLoss(rnn_hidden_null, input[idx], rnn_hidden_right[idx][0], rnn_hidden_rightLoss[idx][0], rnn_hidden_nullLoss,
              inputLossTmp);
        else
          _rnn_right.ComputeBackwardLoss(rnn_hidden_right[idx + 1][0], input[idx], rnn_hidden_right[idx][0], rnn_hidden_rightLoss[idx][0],
              rnn_hidden_rightFLoss[idx + 1][0], inputLossTmp);
