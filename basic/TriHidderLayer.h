...
public:
  ...
  inline void initial(int nOSize, int nISize1, int nISize2, int nISize3, bool bUseB=true, int seed = 0, int funcType = 0) {
     ...
     random(_W2, -1.0 * bound, 1.0 *bound, seed);
     random(_W3, -1.0 * bound, 1.0 *bound, seed);
     random(_b, -1.0 * bound, 1.0 *bound, seed);
   ...
   inline void initial(Tensor<xpu, 2, double> W1, Tensor<xpu, 2, double> W2,
       Tensor<xpu, 2, double> W3, Tensor<xpu, 2, double> b,
...
public:
  inline void ComputeForwardScore(Tensor<xpu, 2, double> &x1, Tensor<xpu, 2, double> &x2,
      Tensor<xpu, 2, double> &x3, Tensor<xpu, 2, double> &y)
  {
    y = dot(x1, _W1.T());
    y += dot(x2, _W2.T());
    y += dot(x3, _W3.T());
  ...
  inline void ComputeBackwardLoss(Tensor<xpu, 2, double> x1, Tensor<xpu, 2, double> x2,
      Tensor<xpu, 2, double> x3, Tensor<xpu, 2, double> y, Tensor<xpu, 2, double> ly,
